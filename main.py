...
df = yf.download(args.ticker, period='max', interval='1wk').dropna()
...
model = NeuralProphet(daily_seasonality=True, yearly_seasonality=True, weekly_seasonality=True)
metrics = model.fit(dfSlim, freq="W")
future = model.make_future_dataframe(dfSlim, periods=48, n_historic_predictions=len(dfSlim))
